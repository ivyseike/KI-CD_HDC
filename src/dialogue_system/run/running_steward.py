...
class RunningSteward(object):
    ...
    def simulate(self, epoch_number, train_mode=False):
        ...
        for index in range(0, epoch_number,1):
            ...
            if train_mode is True:
                ...
                result = self.simulation_epoch(epoch_size=self.epoch_size, index=index)
    ...
    def simulation_epoch(self, epoch_size, index):
        ...
        res = {"success_rate":success_rate, "average_reward": average_reward, "average_turn": average_turn,
               "average_wrong_disease":average_wrong_disease,'average_match_rate':match_rate,'average_match_rate2':match_rate2,"ab_success_rate":absolute_success_rate}
    ...
    def evaluate_model(self, dataset, index):
        ...
        if self.parameter.get("use_all_labels") == True and self.parameter.get("agent_id").lower() in ["agenthrljoint", "agenthrljoint2"] and self.parameter.get('train_mode') == False:
            pickle.dump(self.dialogue_manager.disease_record, open('disease_record.p', 'wb'))
            pickle.dump(self.dialogue_manager.lower_reward_by_group, open('lower_reward_by_group.p', 'wb'))
            pickle.dump(self.dialogue_manager.master_index_by_group, open('master_index_by_group.p', 'wb'))
            pickle.dump(self.dialogue_manager.symptom_by_group, open('symptom_by_group.p', 'wb'))
